...
const I18N_DICT = {
	zh: {
		...
		configUpgradeNoBackupTip:
			"src/config.ts 已升级，但项目备份未成功，请务必手动备份并迁移原配置",
		rollbackBlocked:
			"检测到回滚操作（目标版本 {0} ≤ 当前版本 {1}），不允许降级升级，已终止。",
		backingUp: "正在备份项目...",
		backupDone: "项目已备份至 backup/{0}",
		backupFailed: "项目备份失败，升级将继续但不提供回滚保障",
		...
		confirmYes: "是，开始升级",
		confirmNo: "否，取消",
		confirmHint: "按 Y 确认升级 | N 取消 | ESC 返回",
	...
	en: {
		...
		langZHTW: "繁體中文",
		langJA: "日本語",
		methodTitle: "Select upgrade method:",
		backupWarning:
		...
		configUpgradeNoBackupTip:
			"src/config.ts has been upgraded, but project backup failed, please manually back up and migrate your original config",
		rollbackBlocked:
			"Rollback detected (target version {0} ≤ current version {1}), downgrade is not allowed, aborted.",
		backingUp: "Backing up project...",
		backupDone: "Project backed up to backup/{0}",
		backupFailed:
			"Project backup failed. Upgrade will continue without rollback safety",
		...
		current: "Current",
		zipNotFound:
			"zip command not found, cannot create project backup. Please install:\n  macOS: xcode-select --install\n  Linux: sudo apt install zip",
		unzipNotFound:
	...
	"zh-TW": {
		...
		configUpgradeNoBackupTip:
			"src/config.ts 已升級，但專案備份未成功，請務必手動備份並遷移原配置",
		rollbackBlocked:
			"偵測到回滾操作（目標版本 {0} ≤ 目前版本 {1}），不允許降級升級，已終止。",
		backingUp: "正在備份專案...",
		backupDone: "專案已備份至 backup/{0}",
		backupFailed: "專案備份失敗，升級將繼續但不提供回滾保障",
		...
		confirmYes: "是，開始升級",
		confirmNo: "否，取消",
		confirmHint: "按 Y 確認升級 | N 取消 | ESC 返回",
	...
	ja: {
		...
		configUpgradeNoBackupTip:
			"src/config.ts はアップグレードされましたが、プロジェクトのバックアップに失敗しました。元の設定を手動でバックアップし、移行してください",
		rollbackBlocked:
			"ロールバックが検出されました（ターゲットバージョン {0} ≤ 現在のバージョン {1}）、ダウングレードは許可されていません、中止しました。",
		backingUp: "プロジェクトをバックアップ中...",
		backupDone: "プロジェクトは backup/{0} にバックアップされました",
		backupFailed:
			"プロジェクトのバックアップに失敗しました。アップグレードは続行しますが、ロールバックは保証されません",
		...
		current: "現在",
		zipNotFound:
			"zip コマンドが見つかりません。プロジェクトバックアップを作成できません。先にインストールしてください：\n  macOS: xcode-select --install\n  Linux: sudo apt install zip",
		unzipNotFound:
...
async function backupOnWindows(zipPath, entries) {
	...
	// 写入 UTF-8 BOM，确保 PowerShell 正确识别编码
	// 存储相对路径（用 / 分隔），作为 zip 内的条目名
	fs.writeFileSync(
		tmpList,
...
async function selectMethod() {
	...
	const methodChoice = await selectMenu(
		I18n("methodTitle"),
		[I18n("methodOnline"), I18n("methodLocal")],
		I18n("selectTip"),
	);
	if (methodChoice === BACK) return BACK;
	const isOnline = methodChoice === 0;
...
async function main() {
	...
	banner(
		`${I18n("step")} 1/6: ${isOnline ? I18n("downloading") : I18n("checkingLocal")}`,
	);
	...
	// 让出事件循环，处理 execSync 阻塞期间挂起的 Ctrl+C 信号
	...
	// execSync 阻塞了事件循环，Ctrl+C 可能在备份期间被 PowerShell 吞掉
	// 让出事件循环处理挂起的 SIGINT 信号
	await new Promise((resolve) => setImmediate(resolve));
	if (isInterrupted) {
		exitNow(1);
	}
	...
	const stats = applyUpdate(release, localFiles);
	...
	if (fs.existsSync(configTsPath)) {
		log(
			I18n(backupName ? "configUpgradeTip" : "configUpgradeNoBackupTip"),
			"info",
		);
	}
