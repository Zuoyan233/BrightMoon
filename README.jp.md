...
- ページのスクロールバーのスタイルを追加。
- 天気ウィジェットを追加。7日間の天気予報、IPアドレスによる現在地の自動検出、および手動で都市を検索して他の地域の天気を確認可能。
- Cookieプライバシーポリシー同意バナーを追加。
...
- **天気ウィジェット** - WeatherAPIサービスを採用し、IPアドレスによる現在地の自動検出と手動で都市を検索して他の地域の天気を確認可能。
- **Cookie同意** - Cookieプライバシーポリシー確認バナー。同意/拒否に対応し、拒否時は不要なCookieを自動的に削除。
