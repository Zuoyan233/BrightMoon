...
- 添加外部链接确认弹窗，并魔改部分样式。
- 添加页面滚动条样式。
- 添加天气组件，支持七日天气预报、基于 IP 自动定位当前地区，并支持手动搜索切换查看其他地区天气。
- 添加 Cookie 隐私政策确认横幅。
- 重构个性化配置，新增壁纸模式、樱花特效、文章列表布局、导航栏样式 UI 控制。
- 添加 “跟随系统” 主题模式，支持下拉框选择主题。
- 将 bangumi 和 bilibili 配置整合到 anime 配置项下。
...
- **性能优化** - 懒加载和缓存机制。
- **评论系统** - 支持 Twikoo 集成。
- **翻译组件** - 采用本地 i18n 语言库 + translate.js 实现毫秒级翻译。
- **天气组件** - 采用 WeatherAPI 服务，支持 IP 自动定位当前地区，并支持手动搜索切换查看其他地区天气。
